...
def on_mouse_click (event, x, y, flags, frame):
    ...
    if event == cv2.EVENT_LBUTTONUP:
        ...
        if thiscol == "green":
            ...
            lower_black=np.array([0,0,0])
            upper_black=np.array([180,70,80])
            thiscol = "none"
...
def calibrate():
    ...
    while thiscol != "none":
        ...
        if colors:
            cv2.putText(img, "LAST: "+str(colors[-1]), (10, 100), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 0), 2)
...
while True:
    ...
    try:
        cv2.putText(img, "green: "+str(colors[0]), (10, 50), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 0), 2)
        cv2.putText(img, "orange: "+str(colors[1]), (10, 100), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 0), 2)
        #cv2.putText(img, "black: "+str(colors[2]), (10, 150), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 0), 2)
    ...
    if colors:
        cv2.putText(img, "LAST: "+str(colors[-1]), (10, 100), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 0), 2)
#    cv2.imshow('calibrate'+camid, img)
    cv2.setMouseCallback('robotimg'+camid, on_mouse_click, hsv)
    """
...
#    cv2.imshow("robotimg",robotimg)
    ...
    if redconts and max_area > 5000:
        ...
        redcx_incrop,redcy_incrop = int(M['m10']/M['m00']), int(M['m01']/M['m00'])
...
#        ang = 180/np.pi * np.arcsin((redcy-greency)/(math.sqrt((redcx-greencx)*(redcx-greencx)+(redcy-greency)*(redcy-greency))))
    ...
    if boxY > (ydim-cropsize):
        ...
        Ycropsize = cropsize
    #print(xlen, ylen, boxX, boxY, Xcropsize, Ycropsize)
    ...
    x_end = int(x_min-50*np.cos(lineang*np.pi/180))
    y_end = int(y_min+50*np.sin(lineang*np.pi/180))
    cv2.line(crop_img, (int(x_min),int(y_min)), (x_end,y_end), (200,0,200),2)
    cv2.circle(crop_img,(int(x_min),int(y_min)),3,(200,0,200),-1)
    cv2.line(crop_img, (int(x_min),int(y_min)), (Xcropsize,Ycropsize), (200,0,200),2)
